  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <Button
        ...
        size="sm"
        onClick={handleCopy}
        aria-label={`Copy survey link for ${row.name ?? row.email}`}
      >
      ...
      <Button variant="secondary" size="sm" asChild>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          aria-label={`Open survey link for ${row.name ?? row.email} in a new tab`}
        >
          Open
        </a>
      </Button>
...
export function ProspectsView({
  ...
  const columns: Column<ProspectRow>[] = [
    ...
    {
      key: "link",
      header: <span className="sr-only">Survey link</span>,
      align: "right",
      width: "lg",
