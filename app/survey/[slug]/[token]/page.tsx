...
// A prospect's personal survey link: /survey/[slug]/[token].
//
...
export async function generateMetadata({
  ...
  // prospect in it — see the note in buildSurveyMetadata.
  return buildSurveyMetadata(slug);
}

export default async function ProspectSurveyPage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string; token: string }>;
  ...
  return (
    <SurveyEntry slug={slug} test={test} testEmail={testEmail} src={src} prospect={prospect} />
  );
}
