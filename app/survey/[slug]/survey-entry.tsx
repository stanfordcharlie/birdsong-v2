...
//
//   /survey/[slug]           the generic link (ads, Slack, SMS)
//   /survey/[slug]/[token]   a prospect's personal link
//
// Both render the same InterviewFlow under the same gates; the token route
// only adds a resolved `prospect`, which swaps the anonymous welcome screen
// for a greeted landing page and prefills the intake. Every gate below
// (archived, draft, missing) therefore applies identically to both — which
// is the reason this lives in one file rather than being reimplemented by
// the second route.
...
export const getSurvey = cache(async (slug: string) => {
...
//
// Shared verbatim by the token route: a prospect link's unfurl must look
// exactly like the generic one. It deliberately says nothing about the
// prospect — these previews are generated by mail servers and link scanners
// that are not the recipient, so a greeting here would disclose who was
// invited to anyone the message is forwarded through.
export async function buildSurveyMetadata(slug: string): Promise<Metadata> {
...
export async function SurveyEntry({
  slug,
  test,
  ...
  testEmail?: string;
  src?: string;
  // Already resolved and scoped to this survey by the token route. Null on
  // the generic link, which is the anonymous flow unchanged.
  ...
  //
  // It also outranks a prospect token: an invited prospect arriving after
